...
-- 1. Create an improved version that handles email confirmation
...
-- 3. Create a function to auto-confirm users during beta
...
-- 4. Create trigger to auto-confirm (for beta testing only)
DROP TRIGGER IF EXISTS auto_confirm_on_signup ON auth.users;
CREATE TRIGGER auto_confirm_on_signup
  AFTER INSERT ON auth.users
  FOR EACH ROW EXECUTE FUNCTION public.auto_confirm_user();

-- 5. Test the profile creation with all required fields
DO $$
DECLARE
...
-- 7. Verify the functions exist
SELECT 
  proname as function_name,
  pronargs as num_args
FROM pg_proc
WHERE proname IN ('handle_new_user', 'auto_confirm_user')
ORDER BY proname;