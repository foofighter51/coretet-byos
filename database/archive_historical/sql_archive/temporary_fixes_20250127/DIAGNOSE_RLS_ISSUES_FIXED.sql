-- COMPREHENSIVE RLS DIAGNOSTIC SCRIPT (FIXED)
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'EXPLAIN failed: %', SQLERRM;
END $$;
...
BEGIN
    RETURN QUERY
    SELECT 'tracks'::text, COUNT(*), COALESCE(user_id::text, 'none') FROM tracks GROUP BY user_id LIMIT 1
    UNION ALL
    SELECT 'playlists'::text, COUNT(*), COALESCE(user_id::text, 'none') FROM playlists GROUP BY user_id LIMIT 1;
END;
$$ LANGUAGE plpgsql;
...
-- =====================================================
-- 9. CHECK SPECIFIC USER ACCESS
-- =====================================================
SELECT '=== USER SPECIFIC CHECK ===' as section;

-- If authenticated, check what the current user can see
DO $$
DECLARE
...
BEGIN
    ...
    IF current_user_id IS NOT NULL THEN
        ...
        PERFORM COUNT(*) FROM tracks WHERE user_id = current_user_id;
        RAISE NOTICE 'Tracks owned by current user: %', (SELECT COUNT(*) FROM tracks WHERE user_id = current_user_id);
        
        PERFORM COUNT(*) FROM playlists WHERE user_id = current_user_id;
        RAISE NOTICE 'Playlists owned by current user: %', (SELECT COUNT(*) FROM playlists WHERE user_id = current_user_id);
    ELSE
...
SELECT 
    'Based on the diagnostics above, review:' as note,
    '1. Authentication status - is user logged in?' as check_1,
    '2. JWT email claim - needed for sharing policies' as check_2,
    '3. Query test results - which tables are failing?' as check_3,
    '4. Active queries - any timeouts indicating recursion?' as check_4;